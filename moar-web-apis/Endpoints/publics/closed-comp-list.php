<?php
register_rest_route("v1/", "closed_comp_list", array(
    "methods" => "GET",
    "callback" => "closed_result_competition",
));
function closed_result_competition($request_data) {
    ...
    // pagination code
    if( isset($getparameter['page']) && !empty($getparameter['page']) ) {
        $page = $getparameter['page'];  
    }else{
        $page = 1;
    }
    if(isset( $getparameter["perPage"]) && !empty($getparameter["perPage"]) ){
    ...
    // pagination code end
    $competetion = $wpdb->get_results( "SELECT * FROM wp_Competetion WHERE Date<='" .date("Y/m/d") ."' ORDER BY Id DESC LIMIT {$offset}, {$limitofRecords}" );
    ...
    foreach ($competetion as $Each_competetion) {
        ...
        // To check the available judges for the competition
        $jge1 = !empty($Each_competetion->JudgeOne) ? $Each_competetion->JudgeOne : "";
        $jge2 = !empty($Each_competetion->JudgeTwo) ? $Each_competetion->JudgeTwo : "";
        $jge3 = !empty($Each_competetion->JudgeThree) ? $Each_competetion->JudgeThree : "";
        $jge4 = !empty($Each_competetion->Judge4) ? $Each_competetion->Judge4 : "";
        $jge5 = !empty($Each_competetion->Judge5) ? $Each_competetion->Judge5 : "";
        $jge6 = !empty($Each_competetion->Judge6) ? $Each_competetion->Judge6 : "";
        $jge7 = !empty($Each_competetion->Judge7) ? $Each_competetion->Judge7 : "";
        $jge8 = !empty($Each_competetion->Judge8) ? $Each_competetion->Judge8 : "";
        $jge9 = !empty($Each_competetion->Judge9) ? $Each_competetion->Judge9 : "";
        $jge10 = !empty($Each_competetion->Judge10) ? $Each_competetion->Judge10 : "";
        // The following code sinpepet is to chekc the selected tags for the competition
       ...
       return $get_res;
            ...
            $get_res= new WP_REST_Response(array([     "closed_comp" => $comp,
                                                    "response"     =>  $message,
                                                    "page" => $from,
                                                    "perPage" => $to,
                                                    "total" => $total,
