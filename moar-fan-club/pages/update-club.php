...
.compImg{
    width: 16%; margin-top: -8px; float: left;    padding-left: 40px;
}
.card-moar input, .card-moar select, .card-moar textarea {
    color: white;
...
<!-- 			get update query by id start -->
			<?php
			global $wpdb;
			$competetionGet = $wpdb->get_results("SELECT * FROM wp_Competetion Where JudgeOne ='".get_current_user_id()."' 
					OR JudgeTwo ='".get_current_user_id()."'  OR JudgeThree ='".get_current_user_id()."'  OR Judge4 ='".get_current_user_id()."' 
					OR Judge5 ='".get_current_user_id()."'  OR Judge6 ='".get_current_user_id()."' OR Judge7 ='".get_current_user_id()."'  OR Judge8 ='".get_current_user_id()."'  OR Judge9 ='".get_current_user_id()."'  OR Judge10 ='".get_current_user_id()."' OR createdBy ='".get_current_user_id()."' AND Id='".$_GET["customTableComId"]."' ");
  
      foreach ($competetionGet as $this_competition) {
		  
			$comp_id= $this_competition->Id;
		  $competetionName= $this_competition->competetionName;
		  $pickdateforContent= $this_competition->pickdateforContent;
		  $Date= $this_competition->Date;
		  $media_document= $this_competition->media_document;
		  $createdBy= $this_competition->createdBy;
		  $description= $this_competition->description;
		  
	  }
			?>
       <div class="card-moar">
                    <div class="card-header card-header-primary">
                       <img class="w-100" src="<?php echo get_site_url(); ?>/wp-content/uploads/2021/08/zachary-smith-zorgErvL_Fs-unsplash.png" />
                                        ...
                                        <div class="col-md-2"><label>Name</label></div>
                                              <div class="col-md-10">
                                                ...
                                                <label class="bmd-label-floating"></label>
                                                   <input type="text" name="competetionName" placeholder="Competition Name" 
														 value="<?php echo $competetionName; ?>" id="companyname" class="form-control" required=""  maxlength="50" aria-required="true">
                                                  </div>
                                                </div>
											  </div>

<!-- 									    content upload during this time html end -->
                                                ...
                                                <div class="form-group bmd-form-group">
                                                   <label class="bmd-label-floating"></label> 
                                                     <input required="" type="text" id="pickdateforContent"
                                                        ...
                                                        value="<?php echo $pickdateforContent; ?>"
                                                        autocomplete="off"
                                                        readonly
...
<!-- 														<input type="file"   multiple> -->
													 ...
													 <img src="<?php echo $media_document; ?>" id="frontImage" class="compImg" />
													</div>
													 <div class="file-path-wrapper">
														<input required="" value="<?php echo $media_document; ?>" class="file-path validate" name="bannerImage" id="imageUrl" type="hidden" placeholder="Upload one or more files">
													</div>
                                              </div>
                                         </div>
                                    <div class="row" id="inputFormRow">
                                      <div class="col-md-2"><label>Select Judge</label></div>
                                       <div class="col-md-10 judges_selection">
                                        <select name="selected_judges[]" class="form-control multiselect-comp multiselect_field" multiple="multiple">
                                            <?php foreach($judge as $item): ?>
                                            <option value="<?php echo $item->id; ?>">
                                                <?php echo $item->display_name; ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                        <p class="judges_selection_error">  </p>
                                    </div>
                                  </div>
                                  <div class="row" id="inputFormRow">
                                    <div class="col-md-2"><label>Select Tags</label></div>
                                              <div class="col-md-10">
                                         <select name="selected_tags[]" class="form-control create_new_tags multiselect_field" multiple="multiple">
                                             <?php  
                                                global $wpdb;
                                                $available_tags = $wpdb->get_results("SELECT * FROM wp_competition_tags Where type ='competition_tag'");
                                                if(isset($available_tags) && !empty($available_tags)){
                                ...
                                <div class="row" id="inputFormRow">
                                 <div class="col-md-2"><label>Description</label></div>
                                  <div class="col-md-10 jugSec">
                                    <textarea required="" width="400px" id="textareaChars" style="height:165px" name="description" 
                                      placeholder="Enter Description" maxlength="300" ><?php echo $description; ?></textarea>
							      </div>
								  <span class="text-right" id="chars">300</span>
						        </div>
						       <input required=""  name="createdBy" type="hidden" value="<?php echo get_current_user_id(); ?>"  />
							      <button type="submit" name="submitTimingForm" class="mt-5 ml-3 submitTimingForm btn btn-primary pull-right" 
                                 style="margin-left: 22px;  margin-top: 88px !important; position: relative; z-index: 9999;"> 
								    	Submit
							      </button>
								</div>
                               </div>
...
// 	image upload js
      ...
      <?php if(isset($_POST["submitTimingForm"])){
	                                   	...
	                                   	$JudgeOne=$_POST["title1"];
	                                    $JudgeTwo=$_POST["title2"];
	                                    $JudgeThree=$_POST["title3"]; 
                                        $Judge4=$_POST["title4"]; 
                                       	$Judge5=$_POST["title5"]; 
                                        $Judge6=$_POST["title6"]; 
                                        $Judge7=$_POST["title7"]; 
                                        $Judge8=$_POST["title8"]; 
                                        $Judge9=$_POST["title9"]; 
                                       	$Judge10=$_POST["title10"]; 
	
                                $competetionName=$_POST["competetionName"];
                                $competetionKey=$_POST["competetionKey"];
                                $GetDate=date($_POST["pickdate"]);
                                $GetTime=$_POST["pickTime"];
	                            $pickdateforContent=date($_POST["pickdateforContent"]);
                              ...
                              //  echo $_POST["competitionImage"];
                                global $wpdb;
                            $wpdb->update('wp_Competetion', array(
                                'competetionName' => "$competetionName",
                                'competetionKey' => "$competetionKey",
								'pickdateforContent' => "$pickdateforContent",
								'pickTimeforContent' => "$pickTimeforContent",
                                'Date' => "$GetDate",
								'media_document'=>"$bannerImage",
								// 'JudgeOne'=>"$JudgeOne",
								// 'JudgeTwo'=>"$JudgeTwo",
    ...
    jQuery(document).ready(function($) {
        ...
        $(document).on('change', '.m-input', function() {
            ...
            $('.m-input').each(function() {
                ...
                option = jQuery(this).find('option:selected').val();
                var selected = '';
                if (option != 0) {
                    myarray.push(option);
                }
            ...
            for (let i = 0; i < myarray.length; i++) {
                if (jQuery.inArray(option, myarray) !== -1) {
                    result = true;
                    break;
                }
                map[myarray[i]] = true;
            ...
            if (result) {
                ...
                console.log(' values allowed');
            }
        });

        jQuery("body").on("click", "#addform .submitTimingForm", function() {
            var numItems = jQuery('.m-input').length;
            if (numItems % 2 == 0) {
                console.log("The number is even");
                var validation_error = "The selected judges should always be in odd number.";
                jQuery(".judge_selection_error").text(validation_error);
                jQuery(".judge_selection_error").removeClass("hidden");
            } else {
        ...
        // jQuery(".test_multiselect").bsMultiSelect();
        jQuery('.multiselect-comp').select2({
            closeOnSelect: false,
            maximumSelectionLength: 3 || 5 || 7 || 9 ,
            placeholder: "Select the judges for competition",
        });

        jQuery(".create_new_tags").select2({
            closeOnSelect: false,
            tags: true,
            tokenSeparators: [',', ' '],
            placeholder: "Select tags for the competition",
