...
button, input, optgroup, select, textarea {
                    ...
                    <div class="card-moar">
                        ...
                        <div class="card-body">
                            <form action="" method="post" id="addform" enctype="multipart/form-data">
                                                ...
                                                <div class="form-group bmd-form-group">
                                                    <input type="text"
                                                     name="competetionName"
                                                        placeholder="Competition Name" id="companyname"
                                        ...
                                        <div class="col-md-2"><Label>Notification Date</label></div>
                                            <div class="col-md-4">
                                                <div class="form-group bmd-form-group">
                                        ...
                                        <!-- upload audio or vide record -->
                                                           <!-- banner image -->
                                          <div class="row">
                                            <div class="col-md-2"><Label>Upload Record</label></div>
                                                <div class="col-md-10">
    ...
    <?php if(isset($_POST["submitTimingForm"])){
        ...
        $variable_title = "Judge"; // To create variable judges
        // $counter        = 1; 
        $judges = [];
        foreach($selected_judges as $judgeID) {
            if(isset($judgeID) && !empty($judgeID)) {
                array_push($judges, $judgeID);
                echo $judgeID;
                // $counter++;
            }              
        }

        echo "<pre> The dynamically created value : ";
            print_r($judges[0]);
        echo "<pre>";

        $Judge1 = isset($judges[0]) ? $judges[0] : '';
        $Judge2 = isset($judges[1]) ? $judges[1] : '';
        $Judge3 = isset($judges[2]) ? $judges[2] : '';
        $Judge4 = isset($judges[3]) ? $judges[3] : '';
        $Judge5 = isset($judges[4]) ? $judges[4] : '';
        $Judge6 = isset($judges[5]) ? $judges[5] : '';
        $Judge7 = isset($judges[6]) ? $judges[6] : '';
        $Judge8 = isset($judges[7]) ? $judges[7] : '';
        $Judge9 = isset($judges[8]) ? $judges[8] : ''; 
        $Judge10 = isset($judges[9]) ? $judges[9] : '';  
                            ...
                            if(isset($_POST['selected_tags']) && !empty($_POST['selected_tags'])) {
                                $tags_array         = $_POST['selected_tags'];
                                $tags_array         = array_unique($tags_array); // The function will remove the duplicates from the array
                                $available_tags_new = array();
                        
                                if(isset($tags_array) && !empty($tags_array) && isset($available_tags) && !empty($available_tags)){
                                    foreach($available_tags as $tag){
                                        array_push($available_tags_new, $tag->title);
                                    }
                                    $tags_array = array_merge(array_diff($available_tags_new, $tags_array), array_diff($tags_array, $available_tags_new));
                                    
                                    echo "<pre> The array on the submit page";
                                        print_r(serialize($tags_array));
                                    echo "<pre>";

                                    echo "<pre> The serialized array on the submit page";
                                        print_r(serialize($tags_array));
                                    echo "<pre>";
                                }
                            ...
                            $wpdb->insert('wp_Competetion', array(
    ...
    jQuery(document).ready(function($) {
        ...
        $('.multiselect-comp, .create_new_tags').on('select2:opening select2:closing', function( event ) {
